...
class Servicos extends CI_Controller{
    ...
    public function edit($servico_id){
        ...
        if (!$servico_id || !$this->Core_model->get_by_id('servicos', array('servico_id'=> $servico_id))){
         ...
         redirect('servicos');
            ...
            $this->form_validation->set_rules('servico_nome', '', 'trim|required|min_length[5]|max_length[145]|callback_chek_nome_servico');
    ...
    public function chek_nome_servico ($servico_nome){
        $servico_id = $this->input->post('servico_id');

        if($this->Core_model->get_by_id('servicos', array('servico_nome' => $servico_nome, 'servico_id !=' => $servico_id))){
            $this->form_validation->set_message('chek_nome_servico', 'Esse serviço já existe!');
            return false;
        }else{
            return true;
        } 
